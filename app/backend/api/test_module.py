valid_fetch_types = set(["GPU", "CPU-Gaming", "CPU-Normal"])
...
def test_function(serializer_data):
    if serializer_data["fetch_type"] not in valid_fetch_types:
        return (False, "Not a valid fetch_type")
    ...
    if not set(product_list).issubset(valid_product_set):
        return (False, f"Not a valid {fetch_type} product_list")
